...
class App extends Component {
  ...
  playSound(event) {
    ...
    const audioName = event.currentTarget.id;
  ...
  render() {
    const btnArr = [];
    for (let key of this.state.sounds) {
      btnArr.push(<Button sound={key} playSound={this.playSound} key={key} />)
    }
    return (
